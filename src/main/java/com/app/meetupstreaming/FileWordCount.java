 ...
 * per file and globally, and writes the top N pairs to an output file and to
 * snapshot servers for visualization. Currently designed to work with only 1
 * file at a time; will be enhanced later to support multiple files dropped into
 * the monitored directory at the same time.
 *
 * <p>
 * Receives per-window list of pairs (word, frequency) on the input port. When
 * the end of a file is reached, expects to get an EOF on the control port; at
 * the next endWindow, the top N words and frequencies are computed and emitted
 * to the output ports.
 * <p>
 * There are 3 output ports: (a) One for the per-file top N counts emitted when
 * the file is fully read and is written to the output file. (b) One for the top
 * N counts emitted per window for the current file to the snapshot server and
 * (c) One for the global top N counts emitted per window to a different
 * snapshot server.
 *
...
public class FileWordCount extends BaseOperator {
		...
		public void process(List<WCPair> list) {
			...
			for (WCPair pair : list) {
				...
				if (null != filePair) { // word seen previously in current file
					WCPair globalPair = wordMapGlobal.get(word); // cannot be
																	// null
	...
	public final transient DefaultInputPort<String> control = new DefaultInputPort<String>() {
		...
		public void process(String msg) {
			...
			fileName = msg;
			eof = true;
			// NOTE: current version only supports processing one file at a
			// time.
		}
	};
	...
	public void endWindow() {
		...
		// get topN list for this file and, if we have EOF, emit to fileOutput
		// port
		...
		if (eof) { // got EOF earlier
			if (null == fileName) { // need file name to emit topN pairs to file
									// writer
				throw new RuntimeException("EOF but no fileName at endWindow");
			}

			// so compute final topN list from wordMapFile into fileFinalList
			// and emit it
